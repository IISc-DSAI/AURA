...
async def query_enhancing(user_query):
    ...
    try:
        ...
        # print(f"🧠 Gemma Analysis Response: {type(gemma_analysis)}")
        logging.info(f"Gemma Analysis Response: {gemma_analysis}")
        # Try to parse Gemma's response as JSON
        try:
        ...
        if gemma_analysis.get("needs_youtube", False):
            ...
            if youtube_results.get("success"):
                ...
                if youtube_results['results']:
                    result = youtube_results['results']
        ...
        fallback_response = query_gemma(user_query)
        return {
            ...
            if ask_summarization in ['yes', 'y']:
                for video_url in youtube_videos:
                    try:
                        ...
                        {transcript}
                        """
                        summary = query_gemma(summary_prompt)
